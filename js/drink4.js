...
function updateQuestDisplay(counts) {
    ...
    for (let i = 0; i < target_drinks.length; i++) {
        ...
        if (title_elements[i] && target.count > 0) {
            if (isPicMode) {
                const imgTag = `<img src="${drinkImages[target.name]}" style="height:1.5em; vertical-align:middle; margin-right:1.5dvw;">`;
...
drinks[0].startX = sW * 0.1; drinks[0].startY = sH * 0.68; // barTea
drinks[1].startX = sW * 0.38; drinks[1].startY = sH * 0.68; // stTea
drinks[2].startX = sW * 0.1; drinks[2].startY = sH * 0.39; // orange
drinks[3].startX = sW * 0.38; drinks[3].startY = sH * 0.39; // grape
    ...
    listConfig.forEach(item => {
        if (item.count > 0) {
            if (isPicMode) {
                ...
                const imgTag = `<img src="${drinkImagesById[item.id]}" style="height:1.7em; vertical-align:middle; margin-right:1dvw;">`;
